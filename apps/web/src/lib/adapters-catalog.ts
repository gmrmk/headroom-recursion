...
export const ADAPTERS: ReadonlyArray<AdapterMeta> = [
  ...
  },
  {
    ...
    hint: "MX validate → HIBP breaches → IntelBase (paid) account fanout.",
    examplePayload: '{\n  "email": "alice@example.com"\n}',
    group: "workflow",
  },
  {
    id: "w11.em",
    label: "W11 — Email Deep (free-stack)",
    hint: "MX → HIBP → Gravatar → GitHub commits → Hudson Rock. Free replacement for IntelBase.",
    examplePayload: '{\n  "email": "alice@example.com"\n}',
    group: "workflow",
  },
  {
  ...
  {
    id: "hudson_rock_email_check",
    label: "Hudson Rock — infostealer log check",
    hint: "Free. ~30M+ infected machines. Emits breach-hit per compromised computer with OS + AV + date. Credentials redacted.",
    examplePayload: '{\n  "email": "user@example.com"\n}',
    group: "email",
  },
  {
  ...
  },
  {
    ...
    label: "Microsoft — partial via password reset",
    hint: "Account enumeration via MS account recovery. Per-target investigative use only. Partial value redacted by default; metadata (first char + length + domain) emitted as durable signal.",
    examplePayload: '{\n  "target": "user@example.com"\n}',
    group: "email",
  },
  {
    id: "linkedin_partial_pivot",
    label: "LinkedIn — partial via password reset",
    hint: "Account enumeration; per-target investigative use only. Confirms account existence on LinkedIn + emits redacted partial metadata.",
    examplePayload: '{\n  "target": "user@example.com"\n}',
    group: "email",
  },
  {
    id: "instagram_partial_pivot",
    label: "Instagram — partial via password reset",
    hint: "Account enumeration; per-target investigative use only. STR-host relevance (hosts often link Instagram). Redacted partial metadata.",
    examplePayload: '{\n  "target": "user@example.com"\n}',
    group: "email",
  },
  {
    id: "twitter_partial_pivot",
    label: "Twitter/X — partial via password reset",
    hint: "Account enumeration; per-target investigative use only. Redacted partial metadata.",
    examplePayload: '{\n  "target": "user@example.com"\n}',
    group: "email",
  },
  // Person — name-based
